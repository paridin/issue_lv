defmodule DinoWeb.Live.Example do
  @moduledoc """
  ...
  """
  # instead of use, `use Phoenix.LiveView` this template provides an intragration in dino_web.ex as live_view/0
  # This is why we use: `use DinoWeb, :live_view` similar to `use DinoWeb, :controller` or `use DinoWeb, :view`.
  ...
  def render(assigns) do
    ...
    <section class="mx-10">
      <header class="flex items-center justify-between px-4 py-3 font-bold text-white bg-green-800 rounded shadow-lg">
        <h3 class="text-lg">Live View Example <%= @state.epa %> </h3>
  ...
  defp put_top(socket) do
    case :os.type() do
      {:unix, :darwin} ->
        ...
        assign(socket, top: top, state: %{epa: "yeap"})
      {:unix, :linux} ->
